...
#Procédure
def marche(x,position):         #Fonction permettant l'animation de déplacement du personnage
        if position==sol:
                x+=1
                if x==10:
                        x=0
...
def grosse_def(fenetre,choix):
        ...
        #Base de donnees
        CreateDataBase = sqlite3.connect('Base_de_données.sq3')# on crée une base de donnée temporaire
        QueryCurs = CreateDataBase.cursor()


        def CreateTable(): #on crée une table
            QueryCurs.execute('''CREATE TABLE temps(temps INT)''')

        def AddEntry(temps):# on insère une donnée dans la base
            QueryCurs.execute('''INSERT INTO temps (temps)
            VALUES (?)''',(temps,))
            #on ajoute le temps mis pour vaincre l'adversaire  

        def OrderEntry():
            QueryCurs.execute('''SELECT * FROM temps ORDER BY temps DESC''')
            #on classe les scores présents dans la base de donnée dans l'ordre croissant

        
        #CreateTable()  #si la table n'existe pas
        ...
        personnage1_transition = pygame.image.load(filepath("1_Personnage_deplacement.png")).convert_alpha()   #Chargement des images
        ...
        p2_mouvementcoup65 = pygame.transform.flip(p2_mouvementcoup6,1,0)                              #Fin de chargement
        ...
        hitboxpied1=pygame.Rect(0,0,0,0)                          #Listes des hitboxs des différents coups
        ...
        #INTRO
        #timer
        text = font.render("90",1,(255,196,0))                    #Compte à rebours avant le début du combat
        ...
        while continuer:
                fenetre.blit(fond, (0,0))
                contact1=pygame.Rect(position_personnage2[0],position_personnage2[1],largeur*5/64,hauteur/4) #Zone de conctact pour que les persos ne se rentrent pas dedans
                ...
                for event in pygame.event.get():#Attente des événements
                        if event.type==KEYDOWN:
                                ...
                                if event.key == K_UP:      #Reception des touches
                                        j=1
                                if event.key == K_RIGHT:
                                        if position_personnage2[0]<largeur*3/4 and bumped2==contre2==0: #Initialisation du mouvement pour les déplacements si le personnage n'est pas frappé ou en train de contrer
                                ...
                                if event.key == K_d:
                                        if position_personnage1[0]<largeur*3/4 and bumped1==contre1==0:
                                                vitesse_personnage1[0] = largeur*6/640
                                        liste2.append("c")
                                if (event.key == K_KP9 or event.key == K_p) and coup_poing2==coup_pied2==contre2==0: #Touches pour les coups
                                        coup_poing2=1
                                ...
                                if (event.key == K_KP8 or event.key == K_o) and coup_poing2==coup_pied2==0:
                                        contre2=1
                        if event.type==KEYUP:                           #Informations sur le relachement des touches
                                if event.key==K_RIGHT:
                                        liste1.remove("a")
                                if event.key==K_LEFT:
                                        liste1.remove("b")
                        ...
                        if event.type == QUIT:
                                continuer=0
                if liste1==[]:                          #Liste permettant de savoir quelle touche a été appuyée en premier en allant chercher la dernière valeur de la liste
                    vitesse_personnage2[0] = 0          #Si la liste est vide aucune touche de déplacement n'est pressée le personnage n'a pas de vitesse
                    y=0
                elif liste1[len(liste1)-1]=="a":
                    if position_personnage2[0]<largeur*3/4 and bumped2==contre2==0:
                        vitesse_personnage2[0] = largeur*6/640
                        y=marche(y,position_personnage2[1])      #Appel de la fonction marche pour coller l'image alternative de déplacement pour donner une illusion de mouvement
                    else:
                ...
                if liste2==[]:
                    ...
                    if largeur/64<position_personnage1[0] and bumped1==contre1==0:
                        ...
                        vitesse_personnage1[0]=z=0
                if j==1 and position_personnage2[1]==sol and contre2==0: #Si la touche saut est appuyée ET que le perso touche le sol sans contrer alors il sautera
                        vitesse_personnage2[1] = vitesse_saut
                if k==1 and position_personnage1[1]==sol and contre1==0:
                        vitesse_personnage1[1] = vitesse_saut
                if position_personnage1[1]==sol:                 #Si le personnage touche le sol il subit une force de réaction du sol qui annule la gravité
                        reaction1=-gravite
                        if sol_arret1==0:
                                vitesse_personnage1[1]=0
                                sol_arret1=1                   #Variable permettant de mettre à zéro la vitesse verticale quand le personnage touche le sol pour la première fois, sinon il arreterait d'accélèrer mais aurait une vitesse constante et traverserait le sol
                else:
                        reaction1=0
                        sol_arret1=0
                if position_personnage2[1]==sol:
                        ...
                        reaction2=0
                        sol_arret2=0
                vitesse_personnage1[1]+=(gravite+reaction1)/140    #Application de l'accélèration gravitationnelle à la vitesse verticale des personnages
                vitesse_personnage2[1]+=(gravite+reaction2)/140
                if vitesse_personnage1[1]>=sol-position_personnage1[1]: #Si la vitesse verticale du personnage est > à sa distance au sol alors sa prochaine position sera le sol et non en dessous du sol
                ...
                if coup_poing2==1:                                       #Si le joueur2 a donné un coup de poing cette boucle s'initialise
                        if gauche==personnage1 and bras2==0:             #On regarde si le personnage est à droite ou à gauche de l'autre pour l'orientation du coup pendant toute l'animation
                        ...
                        bras2+=1
                        fenetre.blit(listecoup1[bras2], position_personnage2) #bras2 est la variable qui donne la position dans la liste des images de coups au moment du collage
                        if bras2==7:
                                bras2+=1
                                coup_poing2=2
                if coup_poing2==2:
                ...
                if coup_pied1!=0:   #Presque identique au coup de poing
                        if coup_pied1==1:
                ...
                if g1==personnage1:    #Si le personnage est à gauche il tapera vers la droite et inversement
                ...
                if position_personnage1[0]<position_personnage2[0]:
                        gauche=personnage1 #Le personnage1 est à gauche
                        corps2=pygame.Rect(position_personnage2[0]+largeur*9/128,position_personnage2[1]+hauteur/50,largeur*12/128,hauteur*5/16) #Hitbox du corps
                ...
                hitboxmain2=pygame.Rect(hitbox1[bras2]) #Hitbox de la main
                hitboxmain1=pygame.Rect(hitbox2[bras1])

                if contact2.colliderect(contact1): #Si la hitbox de la main rencontre la hitbox du corps de l'adversaire on regarde si il a bloqué ou si il est déjà tapé, sinon il perd de la vie
                ...
                if hitboxpied2.colliderect(corps1) and coup_pied2!=0 and bumped1==0 and contre1==0:
                        bumped1=1
                        vie1-=8
                if bumped1==1: #Si le personnage1 est frappé il est envoyé en l'air grâce à cette boucle
                        if bouclebumped1==0:
                ...
                if gauche==personnage1: #On regarde dans quelle sens sont les personnages les uns par rapport aux autres afin de coller la bonne image
                        if coup_poing1==0 and coup_pied1==0 and contre1==0:    #On ne colle pas l'image de base ou de contre ou... si le perso donne un coup de poing
                                if 5<z<=10:
                                        fenetre.blit(personnage1_transition2, position_personnage1)
                                else:
                                        fenetre.blit(personnage1, position_personnage1)
                        if coup_poing2==0 and coup_pied2==0 and contre2==0:   #On n'affiche que 1 image pour qu'il n'y ait pas de superposition (ex:coup de poing et coup de pied en même temps)
                                if 5<y<=10:
                ...
                position_personnage1[0]+=vitesse_personnage1[0]   #Déplacement du personnage selon sa vitesse horizontale
